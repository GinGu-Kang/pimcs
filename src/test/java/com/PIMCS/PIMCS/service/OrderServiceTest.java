...
public class OrderServiceTest {
    ...
    @Test
    @Commit
    public void insertOrderMailTest(){
        OrderMail orderMail = OrderMail.builder().managerInfo("나는바보에요").greeting("안녕하십니까").build();
        orderService.insertOrderMail(orderMail);
    }
    @Test
    @Commit
    public void updateOrderMailTest(){
        orderService.updateOrderMail("    <주문자에게온 메일 내용 >  2022년 3월 16일 강성수 hihi\n" +
                "------------------------------------------------------------------------------------------------------------\n" +
                "(주)스마트쇼핑 테크니컬 서포트입니다.\n" +
                "이번에는 재고 관리 IoT 스마트 매트를 신청 해 주셔서 감사 합니다 .\n" +
                "디바이스 대수·배송처 주소·배송 예정일은 이하와 같습니다.","(주)스마트쇼핑 테크니컬 서포트\n" +
